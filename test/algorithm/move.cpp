...
TEST(Algorithm, move) {
	{
		...
		move(from_array.crbegin(), from_array.crend(), make_insert_back_iterator(to_array));
		...
		move(from_list.crbegin(), from_list.crend(), make_insert_back_iterator(to_list));
		...
		move(from_list.cbegin(), from_list.cend(), make_insert_front_iterator(to_list));
