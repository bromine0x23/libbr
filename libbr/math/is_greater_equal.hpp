...
constexpr auto libbr_is_greater_equal_64(BR::Float64 x, BR::Float64 y) -> bool {
	return __builtin_isgreaterequal(x, y);
}
#else
constexpr auto libbr_is_greater_equal_32(BR::Float32 x, BR::Float32 y) -> bool {
...
constexpr auto is_greater_equal(Float32 x, Float32 y) -> bool {
	return libbr_is_greater_equal_32(x, y);
}

constexpr auto is_greater_equal(Float64 x, Float64 y) -> bool {
	return libbr_is_greater_equal_64(x, y);
}

#if defined(BR_GCC)
template< typename TX, typename TY >
constexpr auto is_greater_equal(TX x, TY y) -> EnableIf< BooleanAnd< IsArithmetic<TX>, IsArithmetic<TY> >, bool > {
	return __builtin_isgreaterequal(Common<TX, TY>(x), Common<TX, TY>(y));
}
#else
template< typename TX, typename TY >
constexpr auto is_greater_equal(TX x, TY y) -> EnableIf< BooleanAnd< IsArithmetic<TX>, IsArithmetic<TY> >, bool > {
