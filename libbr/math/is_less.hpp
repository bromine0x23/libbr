...
constexpr auto libbr_is_less_64(BR::Float64 x, BR::Float64 y) -> bool {
	return __builtin_isless(x, y);
}
#else
constexpr auto libbr_is_less_32(BR::Float32 x, BR::Float32 y) -> bool {
...
constexpr auto is_less(Float32 x, Float32 y) -> bool {
	return libbr_is_less_32(x, y);
}

constexpr auto is_less(Float64 x, Float64 y) -> bool {
	return libbr_is_less_64(x, y);
}

#if defined(BR_GCC)
template< typename TX, typename TY >
constexpr auto is_less(TX x, TY y) -> EnableIf< BooleanAnd< IsArithmetic<TX>, IsArithmetic<TY> >, bool > {
	return __builtin_isless(Common<TX, TY>(x), Common<TX, TY>(y));
}
#else
template< typename TX, typename TY >
constexpr auto is_less(TX x, TY y) -> EnableIf< BooleanAnd< IsArithmetic<TX>, IsArithmetic<TY> >, bool > {
